...
The dialog SHALL load available user candidates from the backend, exclude the authenticated user, and let the user select one candidate. On confirmation, the application SHALL create an active `MAIN_ADVISOR` relationship with the selected candidate as advisor and the authenticated user as advisee.

#### Scenario: User saves a selected advisor
- **WHEN** a default account without an active advisor selects a candidate and confirms the selection
...
#### Scenario: No advisor candidate is available
- **WHEN** the backend returns no eligible candidate after excluding the authenticated user
