...
The upload flow SHALL select a typification for the backend document using the most specific available match.

#### Scenario: Typifications are available
- **WHEN** a component upload needs a typification id
- **THEN** the application prefers a tipification matching the group item id, then the group id, then the first returned typification
